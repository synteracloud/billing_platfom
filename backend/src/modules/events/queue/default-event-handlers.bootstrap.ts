...
export class DefaultEventHandlersBootstrap implements OnApplicationBootstrap {
  ...
  onApplicationBootstrap(): void {
    this.eventProcessingRegistry.register('billing.invoice.issued.v1', 'invoice-issued-projection', async (event) => {
      this.logger.log(`Async invoice-issued projection refreshed for invoice ${event.aggregate_id}`);
    });

    this.eventProcessingRegistry.register('billing.payment.settled.v1', 'payment-settled-hooks', async (event) => {
